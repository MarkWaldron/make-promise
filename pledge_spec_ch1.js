...
We are going to wrxite a promise library similar to $q & Q,
...
pass a test, change the next pending test from 'xit' to 'xit'.
This spec is xiterative and opinionated; do the tests in order.
--------------------------------------------------------*/
...
describe('Resolving through a deferral', function(){
  ...
  it('changes xits promise state to "resolved"', function(){
    ...
    attempt fullfillment, for simplicxity's sake pledge.js will treat
...
describe('Rejecting through a deferral', function(){
  ...
  it('changes xits promise state to "rejected"', function(){
    deferral.reject();
...
describe('Settled promises never change state:', function(){
  ...
  it('reject does not overwrxite resolve', function(){
  ...
  it('resolve does not overwrxite reject', function(){
    deferral.reject( 404 );
    deferral.resolve( 'Dumbledore' );
    expect( promise.state ).toBe( 'rejected' );
...
promise parent and manager, which can resolve or reject xits
associated promise.
*/
